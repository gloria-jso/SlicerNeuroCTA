# File from https://github.com/lankabelgezogen/nnunet-angio-infer/tree/main
...
class DC_and_CE_and_CLDC_loss(nn.Module):
    ...
    def forward(
        self, net_output: torch.Tensor, target: torch.Tensor, t_skeletonize_flage=False
    ):
        """
        ...
        cldice_loss = (
            self.cldice(net_output, target, t_skeletonize_flage=t_skeletonize_flage)
...
class CE_and_CLDC_loss(nn.Module):
    def __init__(
        self, ce_kwargs, cldice_kwargs, weight_ce=1, weight_cldice=1, ignore_label=None
    ):
        super(CE_and_CLDC_loss, self).__init__()
        self.cldice = SoftclDiceLoss(**cldice_kwargs)
        self.ce = RobustCrossEntropyLoss(**ce_kwargs)
        self.weight_cldice = weight_cldice
        self.weight_ce = weight_ce
        self.ignore_label = ignore_label
    ...
    def forward(
        ...
        result = self.weight_ce * ce_loss + self.weight_cldice * cldice_loss
        return result