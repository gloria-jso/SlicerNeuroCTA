...
class SoftclDiceLoss(torch.nn.Module):
    def __init__(self, iter_=10, smooth=1.0):
        ...
        self.t_skeletonize = Skeletonize(
            probabilistic=False, simple_point_detection="EulerCharacteristic"
        )
    ...
    def forward(self, y_pred, y_true, t_skeletonize_flage=False):
        ...
        y_pred_fore = torch.max(y_pred_fore, dim=1, keepdim=True)[
            0
        ]  # C foreground channels -> 1 channel
        ...
        with torch.no_grad():
            y_true = (
                torch.where(y_true > 0, 1, 0).squeeze(1).float()
            )  # ground truth of foreground
